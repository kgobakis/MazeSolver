...
(2) Which search algorithm did you choose, and why?
 ...
 it makes the most sense to use the algorithm that goes all the way into each path until
...
public class MazeSolver {
    ...
    public Node[] solve(Graph graph) {
    	helper(graph.nodes[0], graph);
    	solution.add(graph.nodes[graph.numNodes]);
    	Node [] toReturn = new Node [solution.size()];
    	
    	for(int i = 0; i < solution.size(); i++) {
    		int temp = solution.get(0).index;
    		solution.remove(0);
    		toReturn[i] = new Node (temp);
    		toReturn[i].visited = true;
    		toReturn[i].inSolution = true;
    	}
    	
    	return toReturn;
    }
    public boolean helper (Node node, Graph graph) {
    	node.visited = true;

    	if(node == graph.nodes[graph.numNodes]) {

    		return true;
    	}
    	else {
    		for(int i = 0; i < graph.numNodes; i++) {
    		if(graph.adjmatrix[node.index][i] && !graph.nodes[i].visited) {
    			if(helper(graph.nodes[i], graph)) {solution.add(node);}
    			return helper(graph.nodes[i], graph);
    		}
    	}
    return false;
    }
