...
public class MazeSolver {
    ...
    public void helper (Node node, Graph graph) {
    	node.visited = true;
    	if(node == graph.nodes[graph.numNodes]) {
    		graph.nodes[graph.numNodes].visited = true;
    		graph.nodes[graph.numNodes].inSolution = true;
    		solution.add(node);
    		return;
    	}
    	else {
    		for(int i = 0; i < graph.numNodes; i++) {
    		if(graph.adjmatrix[node.index][i] && graph.nodes[i].visited==false) {
    			helper(graph.nodes[i], graph);
    		}
    	}
    }
